...
| KI-036 | low | Phase 15 | open | `XCUIDevice.shared.orientation` is a no-op on the iOS 26.5 simulator: it updates XCTest's screenshot canvas (landscape PNG) but never rotates the simulated device, so `testCaptureLandscapeTable` captures show portrait content sideways with a black band. The app itself rotates correctly — verified manually via Simulator ▸ Device ▸ Rotate + `simctl io screenshot` | Verify landscape manually: launch with `--uitest-autostart`, rotate via the Simulator Device menu, `simctl io screenshot`, then `sips -r 90` to view upright |
...
|---|---|---|---|---|---|
| KI-035 | low | Phase 9 | resolved | Root-caused at the Phase 17 gate (2026-07-11): `testRoundEndCelebrationRenders` and `testMemoryAcrossMultipleRounds` (and the fragile `testMultiRoundSessionContinuesPastFirstRound`) are game-drive UI tests that advance only by tapping the draw pile. Since the pre-existing draw rule (2026-07-06, "no drawing with a legal play"), a draw **no-ops** when the human holds a legal play, so their turns advance on the move timer; a move-timed-out move can play a **wild**, leaving a colour choice pending that the move timer is gated behind — the draw-only loops never resolved it, so the round **stalled** to the budget. Phase 17's correct C10 move-timer change (30s→10s) tripled forced human moves, surfacing this test-harness fragility on every run (52s→160-230s). Not a product regression: the 296 unit tests, incl. 1000-game batches ("0 stuck games, every game has a winner"), all complete. | Fixed the harness, not the engine: the three drive loops now resolve a pending colour choice (`colour-pick-*`), and a `--uitest-fast-timers` launch arg shortens the per-move timer to ~1s for these drive tests (production keeps the full 10s/5s). Verified in isolation: testRoundEndCelebrationRenders 30s ✓, testMultiRound 58s ✓, testMemory 157s ✓ (all previously failing). (2026-07-11) |
...
| KI-032 | medium | Phase 9 | resolved | Landscape orientation supported but never polished; doubled the layout code and was the source of several of the above clipping paths | Locked to portrait-only (`Info.plist`); removed the landscape branch from `GameTableView` entirely (Phase 9 A1, 2026-06-26). **Partially superseded in Phase 15 (2026-07-05) by user request: iPad now supports all orientations; iPhone remains portrait-locked.** Enforced at runtime by `OrientationLockDelegate` because iPadOS 26 ignores the Info.plist `~ipad`/`~iphone` orientation variants (base key is permissive). No separate landscape layout branch — the existing zone layout adapts (wider content block, compact zone gaps) |
