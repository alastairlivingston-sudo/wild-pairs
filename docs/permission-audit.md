...
| 2026-06-26 | Phase 7 — QA hardening (`quality_full.sh`) | Same finding recurs at `SettingsView.swift:84` (line shifted by intervening edits; copy unchanged) after the Phase 6/7 feature work (sound coordinator, timers, Side-to-Side Team Pass, etc.) | Pass — false positive, same disposition as the Phase 5 entry above. Re-verified no network APIs (`URLSession`/`URLRequest`/`Network.`), no new entitlements, no new `Info.plist` usage-description keys, and no background modes were introduced by any Phase 6/7 work. `scripts/check_permissions_minimal.sh` and `scripts/check_project_capabilities.sh` both pass clean. |
| 2026-06-26 | Phase 10 — Neon Arcade UX overhaul (`quality_light.sh`) | `check_no_network_usage.sh` flagged a second "tracking" match at `Theme/Theme.swift:196`, inside the new `NeonSegmented` control: `.tracking(1)`. The `SettingsView.swift` finding from prior phases also still recurs. | Pass — false positive. `.tracking(1)` is SwiftUI's `Text` letter-spacing modifier (visual typography for the segmented control's uppercase label), not analytics/ad tracking. No network APIs, entitlements, or `Info.plist` keys were added in Phase 10 — it is presentational-only (palette, layout, and component restyling). No action needed. |
| 2026-06-27 | Phase 11 — Design parity, elemental retheme, stacking, rule audit (`quality_light.sh`) | Both prior "tracking" findings recur unchanged (`Theme/Theme.swift` `.tracking(1)`, `SettingsView.swift` stats placeholder copy) after the full Phase 11 UI/engine work (A–G). | Pass — false positive, same disposition as above. No network APIs, entitlements, or `Info.plist` keys were touched by Phase 11 — engine changes (draw stacking, starting-card rule, live scoring) are pure `WildPairsCore` logic, and UI changes are visual retheme/layout only. |
| 2026-07-04 | Phase 12 — Elemental Live visual redesign (`quality_light.sh`) | Both prior "tracking" findings recur (`Theme/Theme.swift` `.tracking(1)`, now at line 311 after the glass-surface additions shifted the file; `SettingsView.swift` stats placeholder copy unchanged). | Pass — false positive, same disposition as the Phase 10/11 entries. Phase 12 is presentational-only (element-reactive background, glass chrome, glow styling); no network APIs, entitlements, `Info.plist` keys, or new dependencies were introduced. |
| 2026-07-04 | Phase 13 — Engine rule fixes (final-card penalties, Solo! declaration) + table animations | Both prior "tracking" findings recur unchanged. | Pass — false positive, same disposition as above. Phase 13 touches only `WildPairsCore` game logic and SwiftUI animation code; no network APIs, entitlements, or `Info.plist` keys. |
| 2026-07-04 | Phase 14 — Card face declutter, two-row hand, biggest-win records (`quality_light.sh`) | Both prior "tracking" findings recur unchanged. | Pass — false positive, same disposition as above. Phase 14 is card/hand presentation plus local-only statistics fields (`GameStats.biggestRawWin`/`biggestMultipliedWin` in `wildpairs-stats.json`); no network APIs, entitlements, or `Info.plist` keys. |
| 2026-07-05 | Phase 15 — iPad landscape, draw-pickup UX, AI draw timing (`quality_light.sh`) | Both prior "tracking" findings recur unchanged (`Theme/Theme.swift` `.tracking(1)`, `SettingsView.swift` stats copy). | Pass — false positive, same disposition as above. Phase 15 adds iPad orientation support (`UISupportedInterfaceOrientations` values in `Info.plist` are not permissions and add no capability), UI attention states, and pure-Core AI heuristics; no network APIs, entitlements, or usage-description keys. |
| 2026-07-11 | Phase 17 — Playability & UX pass (gate `quality_full.sh`) | Three "tracking" findings: `Theme/Theme.swift` `.tracking(1)` and `Views/InkFoilFace.swift` `.tracking(w*0.006)` (both SwiftUI letter-spacing), plus the `SettingsView.swift` stats placeholder copy (line shifted by the new "Draw Four challenge" toggle). | Pass — false positive, same disposition as above. Phase 17 adds engine rules (Draw Eight, Draw Four challenge), table animations (cross-table travel, hand-off sweep), the pass-and-play dual-ended view, and **bundled card-art image assets** (`WildPairsApp/SoloCards.xcassets`). Re-verified: no network APIs (`URLSession`/`URLRequest`/`Network.`), no new entitlements, no new `Info.plist` usage-description keys, and no background modes. The bundled art is local resources only — `PrivacyInfo.xcprivacy` `NSPrivacyTracking` stays `false` and `check_privacy_manifest.sh` passes. `check_permissions_minimal.sh` and `check_project_capabilities.sh` pass clean. |

| 2026-07-19 | Phase 19 — Wave A edge HUD + table backgrounds (Mac verification, `quality_light.sh` / `quality_full.sh`) | 23 findings, all pre-existing categories. Net change vs. the pre-Phase-19 baseline (`55e0d06`, 22 findings) is **+1**: `Views/GameEdgeHUD.swift` adds three `.tracking(...)` letter-spacing calls while `Views/GameTableView.swift` drops two (the removed `MoveTimerBar`/`TableStateRail` duplicate), and the "telemetry" hit is the `GameViewModel.swift:691` comment that itself reads "no telemetry, no network". | Pass — false positive, same disposition as above. Verified by diffing the scanner output against the baseline commit: no new pattern category appeared and no genuine network API is present. Phase 19 is presentational plus one `Codable` field — the new `TableBackgroundStyle` persists to the existing local `wildpairs-settings.json` only. Re-verified: no `URLSession`/`URLRequest`/`Network.`, no entitlements files, no new `Info.plist` usage-description keys, no background modes. `check_permissions_minimal.sh`, `check_project_capabilities.sh`, and `check_privacy_manifest.sh` all pass clean (`NSPrivacyTracking` remains `false`). |
